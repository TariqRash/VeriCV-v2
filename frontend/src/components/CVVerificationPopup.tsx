...
export function CVVerificationPopup({ cvData, open, onClose, onConfirm }: CVVerificationPopupProps) {
  ...
  const handleConfirm = async () => {
    ...
    setIsLoading(true)
    try {
      await supabaseHelpers.updateCV(cvData.id, {
        extracted_name: name,
      ...
      console.error("[v0] Failed to confirm CV info:", error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-center">{t("verification.title")}</DialogTitle>
        ...
        <div className="space-y-4 py-4">
          ...
          <p className="text-sm text-muted-foreground text-center pt-2">{t("verification.confirm_subtitle")}</p>
        </div>
